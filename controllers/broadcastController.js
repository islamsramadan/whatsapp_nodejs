...
exports.sendRDBroadcast = catchAsync(async (req, res, next) => {
  ...
  if (req.files && req.files.length > 0) {
    jsonData = req.body.clients.map((item) => {
      let client;

      if (req.files && req.files.length > 0) {
        const defaultFile = req.files.filter(
          (field) => field.fieldname.slice(10).slice(0, -1) === 'default'
        )[0];
        console.log('defaultFile -----------------------------', defaultFile);

        const selectedFile = req.files.filter(
          (field) => field.fieldname.slice(10).slice(0, -1) === item.number
        )[0];

        if (selectedFile) {
          client = { ...item, fileName: selectedFile.filename };
        } else if (defaultFile) {
          client = { ...item, fileName: defaultFile.filename };
        } else {
          client = { ...item };
        }
      } else {
