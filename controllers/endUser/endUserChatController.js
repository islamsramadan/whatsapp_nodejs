...
exports.sendEndUserMessage = catchAsync(async (req, res, next) => {
...

  {
    ...
    try {
      ...
      selectedSession = await Session.findById(
        selectedChat.lastSession
      ).session(transactionSession);
      ...
      if (!selectedSession) {
        ...
        let teamUsers = await Promise.all(
          defaultTeam.users.map(async function (user) {
            let teamUser = await User.findById(user).session(
              transactionSession
            );
            return teamUser;
          })
      ...
      selectedChat.lastMessage = newMessage[0];
      selectedChat.lastSession = selectedSession;
      ...
      return next(
        new AppError('Sending message aborted! Try again later.', 400)
      );
    } finally {
      transactionSession.endSession();
    }
